...
export function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 bg-white border-r border-slate-200 flex flex-col transform transition-all duration-200 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${collapsed ? 'w-16' : 'w-60'}`}>
        {/* Logo */}
        <div className={`flex items-center h-16 border-b border-slate-200 flex-shrink-0 ${collapsed ? 'justify-center px-2' : 'justify-between px-4'}`}>
          {collapsed ? (
            <Logo />
          ) : (
            <>
              ...
              <button className="lg:hidden p-1 rounded-md hover:bg-slate-100 text-slate-500" onClick={() => setSidebarOpen(false)}>
                <X className="w-4 h-4" />
              </button>
            </>
          )}
        ...
        <nav className="flex-1 px-2 py-5 overflow-y-auto space-y-6">
          {navSections.map((section) => (
            <div key={section.label}>
              {!collapsed && (
                <p className="px-2 mb-2 text-[10px] font-semibold tracking-widest text-slate-400 uppercase">{section.label}</p>
              )}
              <div className="space-y-0.5">
                {section.items.map((item) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 ${collapsed ? 'justify-center px-2' : 'px-3'} ${isActive ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`
                    }
                  >
        ...
        <div className="px-2 pb-2 hidden lg:block">
          <button
            ...
            className={`w-full flex items-center gap-2 py-2 rounded-lg text-xs text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors ${collapsed ? 'justify-center px-2' : 'px-3'}`}
          >
        ...
        <div className="p-2 border-t border-slate-200 flex-shrink-0">
          {collapsed ? (
            <div className="flex flex-col items-center gap-2 py-1">
              <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold">{userInitial}</div>
              <button onClick={handleLogout} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-400" title="Sign out">
                <LogOut className="w-3.5 h-3.5" />
              </button>
            </div>
          ) : (
            <div className="group flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer">
              <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{userInitial}</div>
              ...
              <button onClick={handleLogout} className="opacity-0 group-hover:opacity-100 p-1.5 rounded-md hover:bg-slate-200 transition-all" title="Sign out">
          ...
          <div className="flex items-center gap-2">
            ...
            <div ref={quickCreateRef} className="relative">
              <button
                ...
                className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-3 py-1.5 rounded-lg transition-colors shadow-sm"
              >
