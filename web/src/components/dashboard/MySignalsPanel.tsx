...
import type { SignalItem } from '@/hooks/useDashboardData'
...
export function MySignalsPanel({ items }: MySignalsPanelProps) {
      ...
      <ul role="list" className="divide-y divide-gray-100">
        {items.map((item) => (
          <li key={`${item.type}-${item.id}`} className="px-4 py-2.5 hover:bg-gray-50 transition-colors">
            <Link to={item.link} className="flex items-center gap-3 min-w-0">
